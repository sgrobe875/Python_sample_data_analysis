...
plt.title("Compound review sentiment over time (old dataset)")
...
for cat in sentiment_Category:
    ...
    plt.xlabel("Time")
    plt.ylabel("Compound sentiment")
    title = "Compound review sentiment over time for " + cat + " (old)"
    plt.title(title)
...
axbar.set_title('Total Number of Reviews per Month (old)')
...
for cat in reviews:
    ...
    axbar.set_ylabel('Number of Reviews')
    #axbar.set_ylim([0,0.8])
    title = "Total Number of Reviews over time for " + cat + " (old)"
...
with open('data/catMonthCount_one-hundred-thousand.json', 'w') as fout:
